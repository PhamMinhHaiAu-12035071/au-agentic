...
- **`javascript-patterns/`** — hub-and-spoke skill: one catalog `SKILL.md` plus 29 `references/*.md` per tool. The catalog routes tasks to a pattern reference; references are lazy-loaded. Copilot layout differs: `.github/prompts/javascript-patterns.prompt.md` + `.github/prompts/javascript-patterns/*.md` (29), reachable via `#file:`. A self-authored MIT `LICENSE` sits at `packages/templates/javascript-patterns/LICENSE` and fans out to each scaffolded skill folder (DEC-009). Upstream sync is manual via `scripts/sync-upstream-patterns.ts`; template-manifest codegen (`packages/cli/scripts/generate-template-manifest.ts`) emits `src/generated/template-manifest.ts` on prebuild to keep `templates.ts` free of ~121 hand-written imports.
