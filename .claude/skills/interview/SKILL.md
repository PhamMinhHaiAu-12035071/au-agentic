...
argument-hint: "[optional: project or feature context]"
disable-model-invocation: true
allowed-tools: AskUserQuestion Read Glob Grep
---
<!-- au-agentic v1.0.0 | tool: claude -->

Use the `AskUserQuestion` tool to interview me thoroughly and deeply so we can turn a vague requirement into a crystal-clear spec with no material blind spots before planning or implementation.

How to use `AskUserQuestion`:
- Use `type: "select"` with a list of options for multiple-choice questions
- Mark the recommended option with a `"(Recommended)"` prefix on the label
- Use `type: "confirm"` for yes/no decisions
- Use `type: "text"` for open-ended questions that need free-form input
- Use `multiSelect: true` when the user should pick multiple options at once
- Always include an "Other / Not sure, use recommended" option in select lists
